...
class BudgetProvider extends ChangeNotifier {
  ...
  List<BudgetItem> getItemsByEvent(String eventId) {
    return _items.where((i) => i.eventId == eventId).toList();
  }

  BudgetSummary getSummary(String eventId) {
    final eventItems = getItemsByEvent(eventId);
    final totalEstimated = eventItems.fold<double>(
      0, (sum, i) => sum + i.estimatedCost
    );
    final totalActual = eventItems.fold<double>(
      0, (sum, i) => sum + (i.actualCost ?? 0)
    );
    final totalPaid = eventItems
      .where((i) => i.isPaid)
      .fold<double>(0, (sum, i) => sum + (i.actualCost ?? i.estimatedCost));
    ...
    return BudgetSummary(
      ...
      paidItems: eventItems.where((i) => i.isPaid).length,
    );
  }
